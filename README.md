...
tool-heavy decode and adds no persistent model memory.

For Q4_K prefill, each rank now omits peer-owned routed-expert work before
building expert tiles. Static mixed attention uses a single-pass online
softmax on gfx1151, reducing per-block LDS from about 32 KiB to 8 KiB and
reading each KV row once. Both changes add no persistent model memory or wire
...
Install ROCm for `gfx1151` on both nodes, then build this fork identically:

```sh
git clone git@github.com:wkljohn/ds4-strix-halo-tp-odinlink.git
cd ds4-strix-halo-tp-odinlink
make -j"$(nproc)" strix-halo
```

Build the validated OdinLink fork on both nodes. Follow its driver installation
guide for the running kernel; the commands below build the userspace library
and standalone verbs provider used by DS4:
...
cd OdinLink-Five
git checkout 8a77ccbf051b5a615a2b4d9a75ede10af524614a
cmake -S . -B build -DBUILD_VERBS=ON -DBUILD_TRAY=OFF
cmake --build build -j"$(nproc)" \
  --target odl_tb5_verbs odl_tb5_verbs_provider
```

After loading the OdinLink kernel driver on both hosts, verify the device and
link before starting the 153 GiB model load:

```sh
ls -l /dev/odl_tb5_0
./OdinLink-Five/build/cli/odl_tb5_cli --server --device 0       # node 1
./OdinLink-Five/build/cli/odl_tb5_cli --client --device 0 \
  --test latency                                                # node 2
```

Use the same binary, provider, model, and environment on both independent
filesystems. Verify the smaller artifacts every run and the model after each
copy/update:

```sh
export MODEL=/path/to/DeepSeek-V4-Flash-Q4_K.gguf
export PROMPT=/path/to/prompt.txt
sha256sum ./ds4 ./OdinLink-Five/build/verbs/libodl_tb5_verbs.so.0.1.0 "$MODEL"
ssh node2 'sha256sum /path/to/ds4 \
  /path/to/OdinLink-Five/build/verbs/libodl_tb5_verbs.so.0.1.0 \
  /path/to/DeepSeek-V4-Flash-Q4_K.gguf'
```

Set these variables on both nodes. Replace the paths with each node's own
local paths; the filesystems do not need to be shared:

```sh
export DS4_TP_VERBS_LIB=/path/to/OdinLink-Five/build/verbs/libodl_tb5_verbs.so.0.1.0
export LD_LIBRARY_PATH=/path/to/OdinLink-Five/build/lib:/path/to/OdinLink-Five/build/verbs
```
...
```sh
# Node 2: start at the same time as node 1. Relaunch only if its connection
# retry window expires before node 1 finishes loading.
./ds4 -m "$MODEL" --rocm --tensor-parallel --role worker \
  --coordinator 10.4.0.1 5599 --transport rdma -c 4096

# Node 1:
./ds4 -m "$MODEL" --rocm --tensor-parallel --role coordinator \
...
```sh
cmp generated-off-1.txt generated-on-1.txt
cmp generated-off-1.txt generated-on-2.txt
cmp generated-off-1.txt generated-off-2.txt
grep odl_wc_stream_copy_summary coordinator.log worker.log
```
