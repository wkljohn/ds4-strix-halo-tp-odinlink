...
def upstream_pool(keys, gates, valid, ape, pool_size=4):
    keys_t = torch.tensor(keys, dtype=torch.float64).unsqueeze(0)
    gates_t = torch.tensor(gates, dtype=torch.float64).unsqueeze(0)
    valid_t = torch.tensor(valid, dtype=torch.bool).unsqueeze(0)
    ape_t = torch.tensor(ape, dtype=torch.float64)
    ...
    logits = grouped_gates + ape_t[None, None]
    logits = logits.masked_fill(~grouped_valid[..., None], float("-inf"))
    probabilities = torch.nan_to_num(logits.softmax(dim=2))
