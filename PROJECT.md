...
duplicated into each worktree. See `CONTRACT.md` for the enforceable rules.

Performance promotion uses a repeated production anchor plus one compact,
cross-disciplinary long-context screen so kernel work is not selected solely
for a single coding prompt. DSpark remains a separate promotion target with a
same-stack ordinary control.
