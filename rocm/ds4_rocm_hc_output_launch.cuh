...
extern "C" int ds4_gpu_hc_expand_add_split_rows_exact_tensor(
    ...
    uint64_t n_tokens64 = 0u, flat_bytes = 0u, hc_bytes = 0u;
    uint64_t split_bytes = 0u, mix_hc64 = 0u;
    if (!out_hc || !block_out || !block_add || !residual_hc || !split ||
        !cuda_hc_hc_token_count(out_hc, n_embd, n_hc, &n_tokens64) ||
        n_tokens64 == 0u || n_tokens64 > 5u ||
        !cuda_hc_mix_width(n_hc, &mix_hc64) ||
        ...
        !cuda_u64_mul3_checked(n_tokens64, mix_hc64, sizeof(float),
                               &split_bytes) ||
        block_out->bytes < flat_bytes || block_add->bytes < flat_bytes ||
        residual_hc->bytes < hc_bytes || split->bytes < split_bytes) return 0;
    ...
    const float *base = (const float *)split->ptr;
    /* Match ds4_gpu_hc_expand_add_tensor's one-row kernel and offsets. */
    hc_expand_kernel<<<(unsigned)((n_elem + 255u) / 256u), 256u>>>(
            (float *)out_hc->ptr,
            (const float *)block_out->ptr,
            (const float *)block_add->ptr,
            (const float *)residual_hc->ptr,
            base + n_hc,
            base + 2u * n_hc,
            n_embd, n_hc, n_tokens, mix_hc, mix_hc, 1);
    return cuda_ok(cudaGetLastError(),
                   "hc_expand_add_split rows exact launch");
}
