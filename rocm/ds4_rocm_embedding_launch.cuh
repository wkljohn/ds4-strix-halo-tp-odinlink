...
extern "C" int ds4_gpu_embed_token_hc_bf16_tensor(
    ...
    if (!out_hc || !model_map || n_vocab == 0u || token >= n_vocab ||
        n_embd == 0u || n_hc == 0u || (uint64_t)n_embd * n_hc > UINT32_MAX) return 0;
    uint64_t weight_bytes = 0u, out_bytes = 0u;
    if (!cuda_u64_mul3_checked(n_vocab, n_embd, sizeof(uint16_t), &weight_bytes) ||
        !cuda_model_range_fits(model_size, weight_offset, weight_bytes) ||
        !cuda_u64_mul3_checked(n_hc, n_embd, sizeof(float), &out_bytes) ||
        !cuda_tensor_has_bytes(out_hc, out_bytes)) return 0;
    const char *wptr = cuda_model_range_ptr(model_map, weight_offset,
                                             weight_bytes, "token_embd_bf16");
    if (!wptr) return 0;
...
extern "C" int ds4_gpu_embed_tokens_hc_bf16_tensor(
    ...
    if (!out_hc || !tokens_t || !model_map || n_vocab == 0u ||
        n_tokens == 0u || n_embd == 0u || n_hc == 0u) return 0;
    uint64_t weight_bytes = 0u, n = 0u;
    if (!cuda_u64_mul3_checked(n_vocab, n_embd, sizeof(uint16_t), &weight_bytes) ||
        !cuda_model_range_fits(model_size, weight_offset, weight_bytes) ||
        !cuda_tensor_has_i32(tokens_t, n_tokens) ||
        !cuda_u64_mul_checked(n_tokens, n_hc, &n) ||
        !cuda_u64_mul_checked(n, n_embd, &n) ||
        !cuda_tensor_has_f32(out_hc, n)) return 0;
    const char *wptr = cuda_model_range_ptr(model_map, weight_offset,
                                             weight_bytes, "token_embd_bf16");
    if (!wptr) return 0;
