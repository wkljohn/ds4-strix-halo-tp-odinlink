...
ATTN_DECODE_INDEXED_SEQTILE_RESEARCH=${ATTN_DECODE_INDEXED_SEQTILE_RESEARCH:-8}
...
[[ $ATTN_DECODE_INDEXED_SEQTILE_RESEARCH == 0 ||
   $ATTN_DECODE_INDEXED_SEQTILE_RESEARCH == 8 ]] || {
  echo "error: ATTN_DECODE_INDEXED_SEQTILE_RESEARCH must be 0 or 8" >&2; exit 2;
}
...
start() {
  ...
  if [[ $DSPARK == 1 ]]; then
    ...
    decode_env=(
      ...
      DS4_ROCM_ATTN_DECODE_INDEXED_SEQTILE_RESEARCH="$ATTN_DECODE_INDEXED_SEQTILE_RESEARCH")
    support_args=()
  fi
  common+=("${decode_env[@]}")
