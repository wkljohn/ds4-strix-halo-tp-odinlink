...
int main(){
    for(uint32_t t:{1u,15u,16u,17u,33u}){B x(t);ship(x);tile<16>(x);check(hipDeviceSynchronize(),"correctness");size_t n=(size_t)t*x.rows,bytes=n*4;std::vector<float>a(n),b(n);hipMemcpy(a.data(),x.a,bytes,hipMemcpyDeviceToHost);hipMemcpy(b.data(),x.b,bytes,hipMemcpyDeviceToHost);size_t bitdiff=0;double maxabs=0,maxrel=0;for(size_t i=0;i<n;++i){if(std::memcmp(&a[i],&b[i],4))++bitdiff;double d=std::fabs((double)a[i]-b[i]);maxabs=std::max(maxabs,d);maxrel=std::max(maxrel,d/std::max(1e-6,std::fabs((double)a[i])));}std::printf("correctness tokens=%u bitdiff=%zu/%zu max_abs=%.9g max_rel=%.9g\n",t,bitdiff,n,maxabs,maxrel);if(maxabs>1e-4&&maxrel>1e-5)return 2;}
