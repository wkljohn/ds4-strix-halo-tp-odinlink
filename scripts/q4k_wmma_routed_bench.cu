...
// Harness-only paired gate/up experiment.  Each thread retains its strided
// share of both activation halves in registers, while the original one-half
...
__global__ __launch_bounds__(256) static void q4k_wmma_routed_pair_kernel(
 ...
 uint32_t *gate_markers,uint32_t *up_markers) {
    ...
    uint32_t expert=tile_experts[tile],start=tile_starts[tile],count=counts[expert];
    if(count<wmma_min_count){if(dispatch_counters&&blockIdx.x==0&&tid==0)atomicAdd(dispatch_counters+1,1u);return;}
    if(dispatch_counters&&blockIdx.x==0&&tid==0)atomicAdd(dispatch_counters+0,1u);
    uint32_t row0=blockIdx.x*I;float gate_acc[J/16][8]={},up_acc[J/16][8]={};
    for(uint32_t kb=0;kb<xq_blocks;++kb){
      int32_t act_regs[5]={};
      #pragma unroll
      for(int q=0;q<5;++q){int l=tid+q*256;if(l<2*J*YS){int half=l/(J*YS),he=l-half*J*YS,j=he/YS,e=he-j*YS;uint32_t lp=start+j;if(lp<count){uint32_t pair=sorted_pairs[offsets[expert]+lp],tok=pair/n_expert;const int32_t*src=(const int32_t*)&acts[((uint64_t)kb*2+(uint32_t)half)*ntokens+tok];act_regs[q]=src[e];}}}
      #pragma unroll
      for(int q=0;q<5;++q){int l=tid+q*256;if(l<J*YS)sy[l]=act_regs[q];}
      if(wave<8){int r=wave*8+lane/4,txi=(lane%4)*8;if(row0+(uint32_t)r<nrows){const auto&w=gate_weights[((uint64_t)expert*nrows+row0+r)*xq_blocks+kb];
        #pragma unroll
        for(int q=0;q<8;++q){int v=*reinterpret_cast<const int32_t*>(w.qs+4*(txi+q));sx[r*XS+16*((txi+q)/8)+(txi+q)%8]=(v>>0)&0x0f0f0f0f;sx[r*XS+16*((txi+q)/8)+(txi+q)%8+8]=(v>>4)&0x0f0f0f0f;}}}
      if(wave<4){int r=wave*16+lane/2;if(row0+(uint32_t)r<nrows){const auto&w=gate_weights[((uint64_t)expert*nrows+row0+r)*xq_blocks+kb];int ksc=lane&1,sc32=unpack_q4k_scales(reinterpret_cast<const int*>(w.scales),ksc),m32=unpack_q4k_scales(reinterpret_cast<const int*>(w.scales),ksc+2);const uint8_t*sc=(const uint8_t*)&sc32,*mn=(const uint8_t*)&m32;float wd=__half2float(*reinterpret_cast<const half*>(&w.d)),wm=__half2float(*reinterpret_cast<const half*>(&w.dmin));
        #pragma unroll
        for(int l=0;l<4;++l)reinterpret_cast<half2*>(sx+r*XS+64)[4*ksc+l]=__floats2half2_rn(wd*sc[l],-wm*mn[l]);}}
      __syncthreads();
      for(int half=0;half<2;++half){if(half){for(int q=0;q<5;++q){int l=tid+q*256;if(l>=J*YS&&l<2*J*YS)sy[l-J*YS]=act_regs[q];}__syncthreads();}if(wave<4){for(int kk=0;kk<4;++kk){auto A=load_rdna3_mirrored_16x8(sx+wave*16*XS+half*32+kk*8,XS);for(int j0=0;j0<J;j0+=16){auto B=load_rdna3_mirrored_16x8(sy+j0*YS+4+kk*8,YS);i32x8 c={};c=wmma_i8_16x16x16(A,B,c);for(int l=0;l<8;++l){int i=2*l+lane/16,j=j0+lane%16;float2 bd=__half22float2(reinterpret_cast<const half2*>(sy+j*YS)[kk]);float2 ad=__half22float2(reinterpret_cast<const half2*>(sx+(wave*16+i)*XS+64)[half*4+kk]);gate_acc[j0/16][l]+=ad.x*bd.x*(float)c[l]+ad.y*bd.y;}}}}__syncthreads();}
      #pragma unroll
      for(int q=0;q<5;++q){int l=tid+q*256;if(l<J*YS)sy[l]=act_regs[q];}
      if(wave<8){int r=wave*8+lane/4,txi=(lane%4)*8;if(row0+(uint32_t)r<nrows){const auto&w=up_weights[((uint64_t)expert*nrows+row0+r)*xq_blocks+kb];
        #pragma unroll
        for(int q=0;q<8;++q){int v=*reinterpret_cast<const int32_t*>(w.qs+4*(txi+q));sx[r*XS+16*((txi+q)/8)+(txi+q)%8]=(v>>0)&0x0f0f0f0f;sx[r*XS+16*((txi+q)/8)+(txi+q)%8+8]=(v>>4)&0x0f0f0f0f;}}}
      if(wave<4){int r=wave*16+lane/2;if(row0+(uint32_t)r<nrows){const auto&w=up_weights[((uint64_t)expert*nrows+row0+r)*xq_blocks+kb];int ksc=lane&1,sc32=unpack_q4k_scales(reinterpret_cast<const int*>(w.scales),ksc),m32=unpack_q4k_scales(reinterpret_cast<const int*>(w.scales),ksc+2);const uint8_t*sc=(const uint8_t*)&sc32,*mn=(const uint8_t*)&m32;float wd=__half2float(*reinterpret_cast<const half*>(&w.d)),wm=__half2float(*reinterpret_cast<const half*>(&w.dmin));
        #pragma unroll
        for(int l=0;l<4;++l)reinterpret_cast<half2*>(sx+r*XS+64)[4*ksc+l]=__floats2half2_rn(wd*sc[l],-wm*mn[l]);}}
      __syncthreads();
...
static bool run_case(const char *name,uint32_t nt,uint32_t ne,const std::vector<int32_t>&sel,uint32_t timing_bucket=0,uint32_t xb=2,uint32_t pair_bucket=0){
 ...
 q8_K_to_q8_1_mmq_kernel<<<dim3(nt,xb),32>>>(dx,dq,nt,xb);hip_check(hipMemset(wg,0xff,os+4),"poison wg");hip_check(hipMemset(wu,0xff,os+4),"poison wu");hip_check(hipMemset(pg,0xff,os+4),"poison pg");hip_check(hipMemset(pu,0xff,os+4),"poison pu");hip_check(hipMemset(gate_marks,0,ms+4),"clear paired gate markers");hip_check(hipMemset(up_marks,0,ms+4),"clear paired up markers");dim3 wgrid((nr+63)/64,tiles);size_t sh=(16*36+64*76)*sizeof(int32_t),pair_sh=sh;q4k_wmma_routed_kernel<16><<<wgrid,256,sh>>>(dg,dq,wg,dsp,doo,dc,dtt,dte,dts,nt,xb,nr,used,0,NULL,NULL);q4k_wmma_routed_kernel<16><<<wgrid,256,sh>>>(du,dq,wu,dsp,doo,dc,dtt,dte,dts,nt,xb,nr,used,0,NULL,NULL);q4k_wmma_routed_pair_kernel<16><<<wgrid,256,pair_sh>>>(dg,du,dq,pg,pu,dsp,doo,dc,dtt,dte,dts,nt,xb,nr,used,0,NULL,gate_marks,up_marks);hip_check(hipDeviceSynchronize(),"GEMM-only and paired phase");
 std::vector<float> hrg(pc*nr),hru(pc*nr),hrm(pc*nr),hwg(pc*nr),hwu(pc*nr),hpg(pc*nr),hpu(pc*nr),hwm(pc*nr),hrgc(pc*nr),hruc(pc*nr),cpu_g,cpu_u,cpu_m;std::vector<uint32_t> hpgm(pc*nr),hpum(pc*nr);host_routed_reference(hg,hu,hx,sel,hw,ne,used,xb,nr,cpu_g,cpu_u,cpu_m);hip_check(hipMemcpy(hrg.data(),rg,os,hipMemcpyDeviceToHost),"ref gate back");hip_check(hipMemcpy(hru.data(),ru,os,hipMemcpyDeviceToHost),"ref up back");hip_check(hipMemcpy(hwg.data(),wg,os,hipMemcpyDeviceToHost),"wmma gate back");hip_check(hipMemcpy(hwu.data(),wu,os,hipMemcpyDeviceToHost),"wmma up back");hip_check(hipMemcpy(hpg.data(),pg,os,hipMemcpyDeviceToHost),"paired gate back");hip_check(hipMemcpy(hpu.data(),pu,os,hipMemcpyDeviceToHost),"paired up back");hip_check(hipMemcpy(hpgm.data(),gate_marks,ms,hipMemcpyDeviceToHost),"paired gate markers back");hip_check(hipMemcpy(hpum.data(),up_marks,ms,hipMemcpyDeviceToHost),"paired up markers back");uint32_t gg=0,ug=0,pgg=0,pug=0,pgmg=1,pumg=1;hip_check(hipMemcpy(&gg,(const char*)wg+os,4,hipMemcpyDeviceToHost),"gate guard back");hip_check(hipMemcpy(&ug,(const char*)wu+os,4,hipMemcpyDeviceToHost),"up guard back");hip_check(hipMemcpy(&pgg,(const char*)pg+os,4,hipMemcpyDeviceToHost),"paired gate guard back");hip_check(hipMemcpy(&pug,(const char*)pu+os,4,hipMemcpyDeviceToHost),"paired up guard back");hip_check(hipMemcpy(&pgmg,(const char*)gate_marks+ms,4,hipMemcpyDeviceToHost),"paired gate marker guard back");hip_check(hipMemcpy(&pumg,(const char*)up_marks+ms,4,hipMemcpyDeviceToHost),"paired up marker guard back");bool pair_exact=!memcmp(hpg.data(),hwg.data(),os)&&!memcmp(hpu.data(),hwu.data(),os)&&pgg==0xffffffffu&&pug==0xffffffffu&&pgmg==0u&&pumg==0u;for(size_t i=0;i<hpgm.size();++i)pair_exact&=hpgm[i]==1u&&hpum[i]==1u;route&=(gg==0xffffffffu&&ug==0xffffffffu);bool shipping=close_vec("dgate",hrg,cpu_g,.05,.01)&close_vec("dup",hru,cpu_u,.05,.01);bool gemm=shipping&&close_vec("gate",hwg,cpu_g,.05,.01)&close_vec("up",hwu,cpu_u,.05,.01)&&pair_exact;printf("CASE %-10s routing+guards=%s shipping-vs-CPU=%s GEMM-only=%s paired-exact=%s",name,route?"PASS":"FAIL",shipping?"PASS":"FAIL",gemm?"PASS":"FAIL",pair_exact?"PASS":"FAIL");
 ...
 if(timing_bucket||pair_bucket){
  BenchResult separate_wmma=time_launch([&](){q4k_wmma_routed_kernel<16><<<wgrid,256,sh>>>(dg,dq,wg,dsp,doo,dc,dtt,dte,dts,nt,xb,nr,used,0,NULL,NULL);q4k_wmma_routed_kernel<16><<<wgrid,256,sh>>>(du,dq,wu,dsp,doo,dc,dtt,dte,dts,nt,xb,nr,used,0,NULL,NULL);});
...
static bool report_paired_gate(){
 double log_sum=0.0;uint32_t retained=0;bool data_ok=true;
 printf("\nPAIRED GATE/UP WMMA GATE\n");
 for(const auto&r:paired_timings){bool selected=r.xq_blocks==16u&&(r.bucket==22u||r.bucket==48u);bool valid=std::isfinite(r.separate_us)&&std::isfinite(r.paired_us)&&r.separate_us>0.0&&r.paired_us>0.0;double speedup=valid?r.separate_us/r.paired_us:NAN;printf("  bucket=%3u K=%4u selected=%s separate_us=%9.3f paired_us=%9.3f speedup=%7.4fx\n",r.bucket,r.xq_blocks*256u,selected?"yes":"no",r.separate_us,r.paired_us,speedup);if(selected){data_ok&=valid;if(valid){log_sum+=std::log(speedup);++retained;}}}
 if(!data_ok||retained!=2u){printf("  gate=UNDECIDED (requires valid buckets 22 and 48)\n");return false;}
...
int main(){bool ok=true;int base_blocks=0,pair_blocks=0;const size_t kernel_lds=(16*36+64*76)*sizeof(int32_t);hip_check(hipOccupancyMaxActiveBlocksPerMultiprocessor(&base_blocks,q4k_wmma_routed_kernel<16>,256,kernel_lds),"baseline occupancy");hip_check(hipOccupancyMaxActiveBlocksPerMultiprocessor(&pair_blocks,q4k_wmma_routed_pair_kernel<16>,256,kernel_lds),"paired occupancy");printf("COMPILED OCCUPANCY baseline_blocks_per_CU=%d paired_blocks_per_CU=%d baseline_lds=%zu paired_lds=%zu\n",base_blocks,pair_blocks,kernel_lds,kernel_lds);
 ...
 // Production K=4096 timing shapes.  Keep them separate from the historical
